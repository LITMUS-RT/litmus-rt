...
static int usb3503_hw_init(struct usb3503_chip *chip)
{
	...
	ret = gpio_is_valid(chip->pdata->irq_gpio);
	if (ret) {
		ret = gpio_request(chip->pdata->irq_gpio, "irq_gpio");
		if (ret) {
			dev_err(&client->dev, "gpio %d request failed\n",
					chip->pdata->irq_gpio);
			return -EIO;
		}
		ret = gpio_direction_output(chip->pdata->irq_gpio, 1);
		if (ret) {
	...
	ret = gpio_is_valid(chip->pdata->gpio_hub_con);
	if (ret) {
		ret = gpio_request(chip->pdata->gpio_hub_con, "gpio_hub_con");
		if (ret) {
			dev_err(&client->dev, "gpio %d request failed\n",
					chip->pdata->gpio_hub_con);
			return -EIO;
		}
		ret = gpio_direction_output(chip->pdata->gpio_hub_con, 0);
		if (ret) {
			dev_err(&client->dev, "gpio %d set direction failed\n",
						chip->pdata->gpio_hub_con);
			return -EIO;
	...
	// RESET_N = 0 -> 1 : Standby Mode -> Configuration start
	ret = gpio_is_valid(chip->pdata->gpio_reset);
	if (ret) {
		ret = gpio_request(chip->pdata->gpio_reset, "gpio_reset");
		if (ret) {
			dev_err(&client->dev, "gpio %d request failed\n",
					chip->pdata->gpio_reset);
			return -EIO;
		}
		s3c_gpio_setpull(chip->pdata->gpio_reset, S3C_GPIO_PULL_NONE);
		ret = gpio_direction_output(chip->pdata->gpio_reset, 1);
		if (ret) {
			dev_err(&client->dev, "gpio %d set direction failed\n",
						chip->pdata->gpio_reset);
			return -EIO;
		...
		mdelay(10);
		gpio_set_value(chip->pdata->gpio_reset, 0);
		mdelay(10);
...
//	if(val&0x01)	; //Device will remain Hub Mode, PRTPWR Output
//	else			i2c_smbus_write_byte_data(client, USB3503_SP_ILOCK, val|0x01); //Device will remain Hub Mode, PRTPWR Output
//
//	while(1) {
//		mdelay(10);
//		val = i2c_smbus_read_byte_data(client, USB3503_SP_ILOCK);
...
//		if(val&0x01)
//	}
//	i2c_smbus_write_byte_data(client, USB3503_SP_ILOCK, 0x33); //Device will remain Hub Mode, PRTPWR Output
	mdelay(100);
